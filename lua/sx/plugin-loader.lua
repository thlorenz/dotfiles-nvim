...
local default_snapshot = join_paths(get_sx_base_dir(), "snapshots", "default.json")
...
function plugin_loader.load(configurations)
  ...
  local status_ok, _ = xpcall(function()
    ...
    vim.g.colors_name = sx.colorscheme
    vim.cmd("colorscheme " .. sx.colorscheme)
  end, debug.traceback)
  if not status_ok then
...
function plugin_loader.get_core_plugins()
  local list = {}
  local plugins = require "sx.plugins"
...
function plugin_loader.load_snapshot(snapshot_file)
  ...
  if not in_headless then
    vim.notify("Syncing core plugins is in progress..", vim.log.levels.INFO, { title = "sx" })
  end
...
function plugin_loader.sync_core_plugins()
  ...
  vim.api.nvim_create_autocmd("User", {
    ...
    once = true,
    callback = function()
      require("sx.plugin-loader").load_snapshot(default_snapshot)
    end,
  })
